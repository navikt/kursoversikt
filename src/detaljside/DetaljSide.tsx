...
const DetaljSide: FunctionComponent<RouteComponentProps> = props => {
    ...
    useEffect(() => {
        const hentOgSettDetteKurset = async () => {
            ...
            await setDetteKurset(resultat.filter(kurs =>{
    ...
    return (
        <div>
            ...
            <Panel className={"MetaInfoPanel"}>
                ...
                <img className={"MetaInfoPanel__ikon"} src={kalenderIkon} alt="plasseringsikon"/>
                    <Element className={"MetaInfoPanel__infoTekst"}><b>Når:&nbsp;</b></Element>{lagDatoTekst(startTidspunkt,sluttTidspunkt,"MetaInfoPanel__dato")}
                </div>
                {lagPaameldingsfristkomponent(pameldingsfrist)}
                {lagStedkomponent(detteKurset)}
                <div className={"MetaInfoPanel__egenskapTop"}>
                    <img className={"MetaInfoPanel__ikon"} src={kursTypeIkon} alt="plasseringsikon"/>
                    <Normaltekst className={"MetaInfoPanel__infoTekst"}><b>Type kurs:&nbsp;</b>{detteKurset.configurable_custom["Type kurs"]}</Normaltekst>
                </div>
            </Panel>
